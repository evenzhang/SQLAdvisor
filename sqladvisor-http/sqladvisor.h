...
public :
    ...
    SqlAdvisor(char* host,int port,char* username,char* password,char* dbname,char** sqllist){
        ...
        options.query = sqllist;
        options.verbose = 1;

        ICP_VERSION = 1;
        STEP_CNT = 1;
        TABLE_ELEMENT = 0;
        //DRIVED_TABLE = current_lex = TABLE_DRIVERD = NULL;
    }

    void advisor_info_format(const char *fmt,...)
    {
...
/*
    ...
    void mysql_sql_parse_join(TABLE_LIST *join_table) {
        ...
        while ((using_field = using_fields++)) {
            if (options.verbose)
                advisor_info_format("第%d步：开始解析join using条件:%s \n", STEP_CNT++,
                        using_field->ptr());
        ...
        if (type == Item::FUNC_ITEM) {
            ...
            if (join_func->argument_count() == 2) {
                ...
                if ((*item_begin)->type() == Item::FIELD_ITEM
                    ...
                    if (options.verbose)
                        advisor_info_format("第%d步：开始解析join on条件:%s=%s \n",
                                STEP_CNT++,
    ...
    void final_table_drived() {
        if (options.verbose)
            advisor_info_format("第%d步：开始选择驱动表,一共有%d个候选驱动表 \n", STEP_CNT++,
                    TABLE_DRIVERD.size());
        ...
        if (!TABLE_DRIVERD.empty()) {
            ...
            for (drived_it = TABLE_DRIVERD.begin();
                ...
                if (options.verbose)
                    advisor_info_format("第%d步：候选驱动表%s的结果集行数为:%d \n", STEP_CNT++,
    	...
    	if (DRIVED_TABLE != NULL) {
    	    ...
    	    if (options.verbose)
    		advisor_info_format("第%d步：选择表%s为驱动表 \n", STEP_CNT++,DRIVED_TABLE->table_name);
    	} else {
    	    advisor_info_format("第%d步：侯选驱动表个数:%d,但无法确定驱动表 \n", STEP_CNT++,TABLE_DRIVERD.size());
    	    exit(-1);
    	}
        }	
    }
    ...
    void finish_with_error(MYSQL *con) {
        if (options.verbose)
            advisor_info_format("第%d步：SQLAdvisor结束！错误日志:%s \n", STEP_CNT++,
                    mysql_error(con));
        if (con != NULL)
            mysql_close(con);
        exit(-1);
    ...
    POSSBILE_INDEX * find_best_index(char *tablename, char *dbname,
        ...
        if (best_index == NULL) {
            if (options.verbose)
                advisor_info_format("第%d步：SQLAdvisor结束！表中没有任何索引 \n", STEP_CNT++);
            if(index_name) free(index_name);
            exit(-1);
        }
        if(index_name) free(index_name);
    ...
    TABLE_LIST * find_table_by_field(const char * field_name) {
        if (options.verbose)
            advisor_info_format("第%d步：开始解析字段%s是哪一张表 \n", STEP_CNT++, field_name);
        SQL_I_List<TABLE_LIST> table_list = current_lex->table_list;
        TABLE_LIST *table = table_list.first;
        ...
        while (table != NULL) {
            if (!has_table && is_this_table(field_name, table->table_name)) {
                ...
                if (options.verbose)
                    advisor_info_format("第%d步：字段%s存在多个表中 \n", STEP_CNT++,
                            field_name);
                return NULL;
            }
            table = table->next_local;
        ...
        if (options.verbose && this_table)
            advisor_info_format("第%d步：确定字段%s在表%s中 \n", STEP_CNT++, field_name,
                    this_table->table_name);
        else if (options.verbose && !this_table)
            advisor_info_format("第%d步：字段%s不存在任何一张表中 \n", STEP_CNT++, field_name);
        return this_table;
    }
    ...
    TABLE_LIST* mysql_sql_parse_group() {
        ...
        if (group_list.elements > 0) {
            if (options.verbose)
                advisor_info_format("第%d步：开始解析group by 条件 \n", STEP_CNT++);
    ...
    TABLE_LIST* mysql_sql_parse_order() {
        ...
        if (order_elemnts > 0) {
            if (options.verbose)
                advisor_info_format("第%d步：开始解析order by 条件 \n", STEP_CNT++);
    		    ...
    		    break;
                    ...
                    if (isPrimary(((Item_field *) (order_field->item_ptr))->field_name,tbname2->table_name)) {
                        if (order_elment_i == order_elemnts) //当order by c1,c2...中包含主键时,只有当主键在最后位置则忽略,否则无法使用order by索引
                            ...
                            advisor_info_format("第%d步：字段%s是主键同时不在order by列表末尾,放弃处理order by条件 \n",STEP_CNT++,((Item_field *) (order_field->item_ptr))->field_name);
                        return NULL;
                    }
        ...
        while (order_field != NULL) {
            if (order_field->item_ptr->type() == Item::FIELD_ITEM) {
                if (isPrimary(((Item_field *) (order_field->item_ptr))->field_name,
                        table->table_name)) {
                    if (options.verbose)
                        advisor_info_format("第%d步：字段%s是主键并且在列表末尾,忽略不添加 \n",
                                STEP_CNT++,
    	...
    	if( ! is_not_tmp_table(table)){
            ...
            if (index != NULL
                    && isPrimary(index->index_field->field_name->field_name,
                            table->table_name)) {
                if (options.verbose)
                    advisor_info_format("第%d步：表%s 经过运算得到的索引列首列是主键,直接放弃,没有优化建议 \n",
            ...
            if (table->index_field_head != NULL) {
                if (s3.size() == 0) {
                    if (options.verbose)
                        advisor_info_format("第%d步：开始输出表%s索引优化建议: \n", STEP_CNT++,
                                table->table_name);
                    g_string_sprintf(create_index_sql,
                            "alter table %s add index idx_%s(%s)",
                            table->table_name, index_name->str, index_field->str);
                    advisor_info_format("Create_Index_SQL：%s \n",
                            create_index_sql->str);
                } else if (options.verbose) {
                    advisor_info_format("第%d步：索引(%s)已存在 \n", STEP_CNT++,
                            index_field->str);
                }
            } else {
                if (options.verbose)
                    advisor_info_format("第%d步：表%s 的SQL太逆天,没有优化建议 \n", STEP_CNT++,
                            table->table_name);
            }
            table = table->next_local;
    ...
    bool isPrimary(const char *field_name, char *table_name) {
        if (options.verbose)
            advisor_info_format("第%d步：开始验证 字段%s是不是主键。表名:%s \n", STEP_CNT++,
        ...
        if (mysql_num_rows(result) > 0) {
            if (options.verbose)
                advisor_info_format("第%d步：字段%s是主键。表名:%s \n", STEP_CNT++,
                        field_name, table_name);
            is_primary = true;
        } else {
            if (options.verbose)
                advisor_info_format("第%d步：字段%s不是主键。表名:%s \n", STEP_CNT++,
                        field_name, table_name);
        }
        g_string_free(table_exist_index_sql, TRUE);
        mysql_free_result(result);
    ...
    bool is_not_tmp_table(TABLE_LIST * table){
    	...
    	if (options.verbose){
    	    advisor_info_format("第%d步：表%s 是临时表,不进行处理 \n", STEP_CNT++, table->table_name);
    	}
    	return false;
        }
        else
                ...
                if (item_func->argument_count() == 2
                    ...
                    for (Item **child = item_func->arguments(); child <= item_end;
                        ...
                        if ((*child)->type() == Item::FIELD_ITEM) {
                            if (options.verbose)
                                advisor_info_format("第%d步：开始解析where中的条件:%s \n",
    ...
    int run() {
        ...
        while ((query = options.query[i]) != NULL) {
            ...
            while (current_lex != NULL) {
                ...
                if (options.verbose) {
                    advisor_info_format("第%d步: 对SQL解析优化之后得到的SQL:%s \n",
            ...
            if (options.verbose) {
                advisor_info_format("第%d步: SQLAdvisor结束! \n", STEP_CNT++);
            }
            i++;
            STEP_CNT = 1;
        }
